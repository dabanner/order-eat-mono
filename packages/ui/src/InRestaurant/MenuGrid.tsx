...
import type { MenuItem, Restaurant } from "@repo/store/src/restaurantStore"
  ...
  return (
    <View style={[styles.container, isKidsMode && styles.kidsContainer]}>
      {Object.entries(groupedItems).map(([categoryId, items]) => (
        <View key={categoryId} style={[styles.category, isKidsMode && styles.kidsCategory]}>
          <View style={[styles.categoryHeader, isKidsMode && styles.kidsCategoryHeader]}>
            <Text style={[styles.categoryTitle, isKidsMode && styles.kidsCategoryTitle]}>
              {isKidsMode
                ? `${getCategoryEmoji(categoryId)} ${getCategoryName(categoryId)} Time !`
                : getCategoryName(categoryId)}
            </Text>
          </View>
          <View style={[styles.grid, isKidsMode && styles.kidsGrid]}>
            {items.map((item) => (
              <MenuItemComponent
                key={item.id}
                item={item}
